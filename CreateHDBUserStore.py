...
class HDBUserStoreClass(object):
	...
	def __init__(self, password):
		...
		if not self.is_mdc:
			...
			self.dparameters['tenantsqlport'] = subprocess.check_output("hdbnsutil -printSystemInformation | awk -v c=2 '/{0}/{{print $c}}' | grep "
			                                                            "--only-matching '.....$'".format(self.dparameters['tenantsid']),
			                                                            shell=True).replace('\n', '')
